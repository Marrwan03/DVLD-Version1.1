    ...
    public partial class ctrlPersonCardWithFilter : UserControl
    {
       ...
       public bool ShowAddPerson
        ...
        public int? PersonID  
        {
            ...
            get { return ctrlPersonCard1.PersonID.Value; }   
        }
        ...
        {
            //if (!clsGlobal.CurrentUser.CheckAccessPermision(clsUser.enPermision.ShowPersonDetailsPeople))
            //{
            //    clsGlobal.PermisionMessage();
            //    return;
            //}
            ...
            {
                PersonID = clsPerson.Find(txtFilterValue.Text).PersonID.Value;
            }
            else
                PersonID = int.Parse(txtFilterValue.Text);

                int? UserID = clsUser.GetUserIDByPersonID(PersonID);
            if (UserID.HasValue)
            {
                string Type = "User";
                string Message = $"This Person has {Type} Acc, Go to {Type}`s section and search about him.";
                int? EployeeID = clsEmployee.GetEmployeeIDByUserID(UserID.Value);
                if (EployeeID.HasValue)
                {
                    Type = "Employee";
                }
